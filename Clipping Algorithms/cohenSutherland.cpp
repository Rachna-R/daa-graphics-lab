...
void cohenSutherland()
{
	for(int i = 0; i < pointsDrawn; i+=2)
	{
		...
		{
			cout << "The line is not completely outside.\n";
			int r = checkInside(bit1, bit2);
			if(r == 1)
			{
				cout << "The line is completely inside\n";
				continue;
			}
			else
...
void keyboardPress(unsigned char key, int x, int y)
{
	switch(key)
	{
		case  's' : cohenSutherland();                            //if 's' character id pressed, then start line clipping
			break;
		default : cout << "\nUnknown Key " << endl;
	}
}
